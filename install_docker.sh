...
#######################################################
    ...
    if [[ "$OS" == [234] ]]; then
        ...
        sudo apt install curl wget git htop nano unzip tar gnupg lsb-release net-tools dnsutils iputils-ping ca-certificates lsof grep -y >> ~/docker-script-install.log 2>&1
